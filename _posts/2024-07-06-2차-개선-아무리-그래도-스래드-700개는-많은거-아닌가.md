...
그래도 좀 더 정확하게 파악하기 위해 엘리베이터 콜 이력 개수와 스래드 개수 그래프를 대조해보면 서로 유사한 그래프가 나온다.
...
netty는 EventLoopGroup은 한개 또는 여러개의 EventLoop를 포함한다. (일종의 스래드 풀 개념)
...
class NettyService(
    ...
    fun lobbyCall(
        ...
        val eventLoopGroup = NioEventLoopGroup() // 요뇨석 이 문제임
...
그렇다는건… 해당 JVM에서 사용할 수 있는 프로세스의 2배 만큼 스래드가 생성되고 있었다는 이야기기 된다.
...
위와 같은 조건으로 테스트를 한 결과 thread가 튀는 형상은 발생하지 않았다.
사진과 같이 스래드 개수는 일정하게 유지 되었다.
...
기존 대 비 41.1% 정도 감소했다.
아무래도 엘리베이터 콜당 EventLoopGroup객체를 생성했으니 Heap Size에 영향이 간게 아닐까 추측한다.

Total Thread 지표에서 최대 720까지 튀던 스래드 개수가 현재는 Total Thread max 개수가 200개 정도로 약 72.22% 정도로 감소했다.

1, 2차 개선 모두 합하면 90% 정도로 스래드 개수가 감소되었다.
