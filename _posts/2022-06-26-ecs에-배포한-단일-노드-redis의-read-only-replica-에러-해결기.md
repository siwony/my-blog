...
## 5. 문제 원인이 뭐였을까? (추정)
아직 redis에 대한 depth가 낮아 원인을 정확하게 모르겠지만, 취업한 친구의 사수님 피셜로 같은 ECS서비스를 사용하면 Redis컨테이너끼리 묶이는 경향이 있다고 한다. 정확한건 나중에 깊게 공부하다 보면 알 수 있지 않을까?

2025-11-09 에 다시한번 생각해 보았지만 에초에 영속성을 위한 서비스가 아니므로 redis ecs를 사용하는게 올바르지 않아보인다.