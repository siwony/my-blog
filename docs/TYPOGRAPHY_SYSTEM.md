...
모든 토큰은 `:root`에 정의되어 있으며 `assets/css/style.css` 파일에서 관리됩니다.
...
- **메인 스타일시트**: `assets/css/style.css`
- **토큰 정의 위치**: `assets/css/style.css` 파일의 `:root` 섹션

---

## 📝 토큰 수정 시

토큰 값을 수정해야 할 경우, `assets/css/style.css`의 `:root` 섹션에서 해당 변수만 수정하면 전체 사이트에 자동 적용됩니다.
