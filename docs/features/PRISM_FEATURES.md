...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 모든 pre 태그에 line-numbers 클래스 자동 추가
...
- **자동 언어 로딩**: 사용된 언어만 동적 로딩 (20개 언어 지원)
- **로컬 호스팅**: 외부 의존성 없는 안정적 서비스
- **Jekyll 통합**: Rouge와의 충돌 없는 완전한 대체

### 🎯 지원 언어 목록
현재 로컬에서 지원하는 20개 언어:
- **Web**: HTML/XML, CSS, JavaScript, JSON, HTTP
- **Backend**: Java, Python, Ruby, C/C-like, Bash
- **Build**: Gradle, Groovy, Properties
- **Data**: YAML, SQL, Protobuf
- **Documentation**: Markdown
- **JVM**: Kotlin

### 🎯 핵심 장점
- **안정성**: 외부 CDN 의존성 없는 로컬 호스팅
- **성능**: 필요한 언어만 로딩하는 효율적 구조  
- **확장성**: 언어 컴포넌트 추가로 쉬운 확장
...
- **로컬 호스팅**: 외부 CDN 의존성 제거로 안정성 확보
- **선택적 로딩**: 필요한 언어 컴포넌트만 동적 로딩
- **Minified Assets**: 모든 JS/CSS 파일 최적화
- **캐싱**: 브라우저 캐싱 활용
- **No Conflicts**: Jekyll 빌드 프로세스와 충돌 없음
- **제한적 언어**: 20개 핵심 언어로 번들 크기 최적화
...
### 언어 확장 계획
우선순위에 따른 언어 추가 계획:
1. **Phase 1**: Go, Rust, TypeScript, PHP, Swift
2. **Phase 2**: Docker, Terraform, GraphQL, R, Scala  
3. **Phase 3**: 기타 요청 언어
