...
### 0. 🧭 AI Ops Hub 먼저 확인 (MANDATORY)

- 모든 작업은 [`docs/ai/`](../ai/)의 `CHECKLIST`와 `TASK_PLAYBOOK`에서 시작합니다. 필요한 세부 규칙은 아래 가이드라인 본문을 참조합니다.
...
#### 작업 전 체크리스트:
1. 🔍 **AI Ops Hub 확인**: `docs/ai/CHECKLIST.md` → `TASK_PLAYBOOK.md` 순으로 기본 절차 점검
2. 📚 **가이드라인 검토**: 해당 작업과 관련된 가이드라인 문서 확인
3. 📝 **표준 준수**: 정의된 코딩 표준 및 네이밍 컨벤션 적용
4. 🧪 **테스트 계획**: 테스팅 전략에 따른 테스트 케이스 작성
5. 🚀 **성능 고려**: 성능 가이드라인에 따른 최적화 적용
